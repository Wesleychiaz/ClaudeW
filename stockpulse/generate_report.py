...
def build_report(data):
    ...
    for n in s.get("recent_news", [])[:5]:
        news_rows += f'<li>{n.get("date","")}&nbsp;&nbsp;{n.get("title","")}</li>'
    if not news_rows:
...
<style>
  ...
  *{{margin:0;padding:0;box-sizing:border-box}}
  ...
  .news-list li{{font-size:13px;color:#888;border-bottom:1px solid #2a2a2a;padding-bottom:8px;line-height:1.5}}
  ...
  .analysis-section p{{font-size:14px;color:#888;line-height:1.7;margin-bottom:12px}}
  .analysis-section ul{{list-style:none;display:flex;flex-direction:column;gap:6px;margin-bottom:12px}}
  .analysis-section ul li{{font-size:14px;color:#888;line-height:1.6;padding-left:14px;position:relative}}
